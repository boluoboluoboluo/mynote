...
function add_down_button(video){
	...
	down_ele.onclick = ()=> {					//下载按钮点击事件
		...
		video_loaded_listener = async function video_loaded(){					//创建监听器
			chrome.storage.local.set(			//关闭网络请求监听
				{'listen_status':0}
			)
			const result = await get_storagedata('listen_urls')
			const listen_urls = result['listen_urls']
			console.log('listen_urls',listen_urls)
			url_box.innerHTML = ""
