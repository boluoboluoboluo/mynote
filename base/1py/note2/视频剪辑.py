...
class vidcut:
	def __init__(self):
		...
		mergefilesinput.insert(0,"1.ts|2.ts|3.ts")
		...
		mergebtn = tk.Button(self.root,text="仅合并为ts",command=lambda:self.merge_vidfile(self.merge_filevar.get().strip(),"ts"))
		mergebtn.grid(row=rowcount,column=1,padx=(10,0),sticky="w")
	...
	def merge_vidfile(self,files_str,suffix):
		...
		command = 'ffmpeg -i "concat:' + files_str +  '" -c copy ' + outfile
		print(command)
		subprocess.run(command)
		print("merge done ...")
